## Currated list More than 75 leetcode questions to save your time updated
...
-  `Median of two sorted arrays`: strong intuition gotten from [here](https://leetcode.com/problems/median-of-two-sorted-arrays/discuss/2471/Very-concise-O(log(min(MN)))-iterative-solution-with-detailed-explanation) time complexity `O(log(min(n,m)))`
...
- `maximum envelope`: Time complexity of O($n^2$ + $n$log(n)) can be improved to O(nlog(n)) if we use Longest increasing Subsequence on the height only
